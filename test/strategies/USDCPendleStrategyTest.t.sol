...
contract USDCPendleStrategyTest is BasePendleStrategyTest {
    ...
    function test_PT_price_BeforeExpire() public {
        ...
        uint32 _twap = 1800;
        _addPTMarket(address(MARKET_ADDR1), UNDERLYING_YIELD_ADDR1, YIELD_TOKEN_FEED1, _twap);
        ...
        assertEq(_ptPrice, (uint256(_yieldUSDPrice) * _ptToSyPrice / uint256(_usdcUSDPrice)));
    }
    ...
    function _addPTMarket(
        ...
        vm.startPrank(strategyOwner);
        PendleStrategy(myStrategy).addPT(_pendleMarket, _underlyingYieldToken, _underlyingOracle, _twapSeconds);
        vm.stopPrank();
    }
    ...
    function _getRedeemAfterExpireCalldataFromSDK(address _pendlePT, uint256 _ptAmount) internal view returns(bytes memory){
        bytes memory _callData;
        if (_pendlePT == address(PT_ADDR2) && _ptAmount == magicPTAmount) {
            // slippage 1% with aggragator enabled
            _callData =
